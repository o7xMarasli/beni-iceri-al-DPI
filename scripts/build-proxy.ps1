# beni-iceri-al-DPI proxy (SpoofDPI 1.2.1) - PowerShell ile derleme ve kopyalama
...
$OutExe = Join-Path $Root "spoofdpi\beni-iceri-al-DPI-proxy.exe"
...
Write-Host "SpoofDPI (beni-iceri-al-DPI-proxy) derleniyor..." -ForegroundColor Cyan
...
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "beni-iceri-al-DPI-proxy-x86_64-pc-windows-msvc.exe") -Force
Write-Host "Kopyalandi: src-tauri\binaries\beni-iceri-al-DPI-proxy-x86_64-pc-windows-msvc.exe" -ForegroundColor Green
