; ═══════════════════════════════════════════════════════════
; beni-iceri-al-DPI NSIS Installer Hooks
; ═══════════════════════════════════════════════════════════

; ─── KURULUM ÖNCESİ ───
!macro NSIS_HOOK_PREINSTALL
    ; Eski sürüm çalışıyorsa kapat
    nsExec::ExecToStack 'taskkill /F /IM beni-iceri-al-DPI.exe'
    Pop $0
    nsExec::ExecToStack 'taskkill /F /IM beni-iceri-al-DPI-proxy.exe'
    Pop $0
    Sleep 500
    ...
    ; Sentinel temizle
    Delete "$TEMP\beniicerialdpi_proxy_active.lock"
    Delete "$TEMP\beniicerialdpi_sidecar.pid"
!macroend
...
!macro NSIS_HOOK_PREUNINSTALL
    ...
    nsExec::ExecToStack 'netsh advfirewall firewall delete rule name=beni-iceri-al-DPI_Proxy'
    Pop $0
    nsExec::ExecToStack 'netsh advfirewall firewall delete rule name=beni-iceri-al-DPI_PAC'
    Pop $0

    ; 5. Autostart registry kaydını temizle
    DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "beni-iceri-al-DPI"
