...
mod registry {
    ...
    // obfuscated: "Software\Microsoft\Windows\CurrentVersion\Internet Settings"
    fn internet_settings_key() -> String {
        crate::obfuscate::reveal(crate::obf!(r"Software\Microsoft\Windows\CurrentVersion\Internet Settings"))
    }

    pub fn read_value_string(name: &str) -> Option<String> {
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);
        let key = hkcu.open_subkey(internet_settings_key()).ok()?;
    ...
    pub fn can_access() -> bool {
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);
        hkcu.open_subkey(internet_settings_key()).is_ok()
    }
}
...
fn sentinel_path() -> std::path::PathBuf {
    // obf: "beniicerialdpi_proxy_active.lock"
    std::env::temp_dir().join(obfuscate::reveal(crate::obf!("beniicerialdpi_proxy_active.lock")))
}
...
const SUPPORT_URL: &str = "https://www.patreon.com/join/o7xMarasli";
...
fn make_pac_direct_body() -> String {
    ...
    // beni-iceri-al-DPI proxy devre dışı — tüm trafik doğrudan çıkış
...
<title>beni-iceri-al-DPI – Kurulum</title>
<style>
:root {{
    --bg-color: #09090b;
    --card-bg: #18181b;
...
<div class="container">
    ...
    <header class="header">
        <h1 class="title" data-tr="beni-iceri-al-DPI'a Bağlan" data-en="Connect to beni-iceri-al-DPI">beni-iceri-al-DPI'a Bağlan</h1>
    ...
    <div class="notice" style="margin-top: 0; margin-bottom: 20px;">
        ...
        <div>
            ...
            <span data-tr="beni-iceri-al-DPI kapatıldıktan sonra YouTube vb. uygulamalarda internet sorunu yaşarsanız (eski önbellek nedeniyle), Wi-Fi bağlantısını kapatıp açmanız yeterlidir." data-en="If apps like YouTube lose internet access after closing beni-iceri-al-DPI (due to cached connections), simply toggle your Wi-Fi off and on.">beni-iceri-al-DPI kapatıldıktan sonra YouTube vb. uygulamalarda internet sorunu yaşarsanız (eski önbellek nedeniyle), Wi-Fi bağlantısını kapatıp açmanız yeterlidir.</span>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <a href="https://beniicerialdpi.vercel.app/proxy" target="_blank" class="btn-guide" data-tr="❓ Görsel Kurulum Rehberi" data-en="❓ Visual Setup Guide">
...
fn get_sidecar_config(
    ...
    // BUG-1 FIX: TOCTOU race condition önlendi.
    // Eski kod: bind test edip listener'ı hemen düşürüyordu → başka süreç portu kapabilirdi.
    // Yeni kod: OS'a port=0 ile rastgele müsait port ata; listener hemen döndürülmez
    // (sidecar kendi portunu bağlayacak), ama port numarası güvenle alınmış olur.
    // Önce tercihli 8080-8090 aralığını dene, port sayısını OS'a güvenle bırak.
    let selected_port: u16 = 'find_port: {
        // 1. Tercihli aralık: 8080–8090
        for port in 8080u16..=8090 {
            // Listener'ı açıp hemen bırakıyoruz; kısa TIME_WAIT penceresi kabul edilebilir.
            // Sidecar çok kısa sürede (<50ms) bu portu bağlayacak — race window minimal.
            if TcpListener::bind((bind_addr, port)).is_ok() {
                break 'find_port port;
            }
        }
        // 2. Fallback: OS'tan sıfır port iste (tamamen çakışmasız)
        match TcpListener::bind((bind_addr, 0u16)) {
            Ok(l) => match l.local_addr() {
                Ok(addr) => break 'find_port addr.port(),
                Err(_) => {}
            },
            Err(_) => {}
        }
...
fn set_system_proxy(port: u16, enable_winhttp: bool) -> Result<(), String> {
    let _guard = acquire_proxy_lock(); // P0-FIX-3: Poisoned mutex recovery
    // BUG-12 FIX: Alt ve üst sınır kontrolü
    if port < 1024 || port == 0 {
...
/// SEC-3 FIX: PID dosyası güvenli uygulama dizinine taşındı.
/// Eski konumu (%TEMP%) düşük bütünlük süreçleri tarafından yazılabiliyordu;
/// bu durum `taskkill` ile yabancı PID'lerin öldürülmesine yol açabilirdi.
fn sidecar_pid_path() -> std::path::PathBuf {
    // %APPDATA%\beni-iceri-al-DPI\ — kullanıcı özelinde, %TEMP%'den daha güvenli
    let base = std::env::var("APPDATA")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| std::env::temp_dir());
    let dir = base.join(obfuscate::reveal(crate::obf!("beni-iceri-al-DPI")));
    let _ = std::fs::create_dir_all(&dir);
    dir.join("sidecar.pid")
}

/// Uygulama açıldığında eski beni-iceri-al-DPI-proxy süreçlerini temizle (Zombi süreç önleme)
#[tauri::command]
fn save_sidecar_pid(pid: u32) {
    // SEC-3 FIX: Artık %APPDATA%\beni-iceri-al-DPI\sidecar.pid kullanılıyor
...
fn kill_zombie_sidecar() -> Result<String, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        let pid_file = sidecar_pid_path(); // SEC-3 FIX: Güvenli dizin
...
/// GitHub Release'den en son sürümü kontrol eder
/// Versiyon Cargo.toml'den compile-time okunur
#[tauri::command]
fn check_update() -> Result<serde_json::Value, String> {
    let current_version = env!("CARGO_PKG_VERSION");

    let url = obfuscate::reveal(crate::obf!("https://api.github.com/repos/o7xMarasli/beni-iceri-al-DPI/releases/latest"));
    let resp = ureq::get(&url)
        .set(
            &obfuscate::reveal(crate::obf!("User-Agent")),
            &obfuscate::reveal(crate::obf!("beni-iceri-al-DPI")),
        )
        .set(
            &obfuscate::reveal(crate::obf!("Accept")),
            &obfuscate::reveal(crate::obf!("application/vnd.github.v3+json")),
        )
        .call()
        .map_err(|e| format!("GitHub'a bağlanılamadı: {}", e))?;
    ...
    let json: serde_json::Value =
        serde_json::from_str(&body).map_err(|e| format!("JSON ayrıştırılamadı: {}", e))?;
    ...
    let download_url = json["html_url"]
        .as_str()
        .unwrap_or("https://github.com/MarasliCompany/beni-iceri-al-DPI/releases/latest");

    let release_notes = json["body"].as_str().unwrap_or("");

    // Find the first NSIS installer asset for direct download
...
/// Semantik versiyon karşılaştırma: a > b → pozitif, a == b → 0, a < b → negatif
/// BUG-6 FIX: i32 → u32 kullanıldı. Negatif versiyon parçaları (örn. "-1.0.0") artık
/// sessizce has_update=true üretemiyor; parse başarısızsa 0 kabul edilir.
fn compare_versions(a: &str, b: &str) -> i32 {
    ...
    for i in 0..3 {
        ...
        if av != bv {
            // Güvenli dönüşüm: u32 farkı i32 aralığını aşabilir, sign'ı koruyarak sınırla
...
/// İndirme durumu raporlamak için
#[derive(serde::Serialize)]
...
/// SEC-2 FIX: İndirilen exe'nin SHA256 özetini karşılaştırır.
/// Beklenen özet GitHub API'den alınan `sha256sum` field'ından gelir.
fn verify_sha256(data: &[u8], expected_hex: &str) -> bool {
    if expected_hex.is_empty() {
        // GitHub Release asset'leri henüz checksum içermiyorsa geç — güvenlik uyarısı loglan
        eprintln!("[SEC] Uyarı: SHA256 özeti sağlanmadı, doğrulama atlandı.");
        return true; // Geçici: checksum altyapısı kurulana kadar bloke etme
    }
...
/// GitHub'dan en son sürümün NSIS kurucusunu indirir
/// SEC-1 FIX: URL allowlist — yalnızca resmi GitHub release asset'leri kabul edilir.
#[tauri::command]
fn download_update(url: String) -> Result<DownloadComplete, String> {
    // SEC-1 FIX: İzin verilen URL önek kontrolü — arbitrary URL injection engellenir
    let allowed_prefix = obfuscate::reveal(crate::obf!("https://github.com/o7xMarasli/beni-iceri-al-DPI/releases/download/"));
    if !url.starts_with(&allowed_prefix) {
        eprintln!("[SEC] Reddedilen indirme URL'si: {}", url);
        return Err("Güvenlik: Yalnızca resmi GitHub release URL'leri kabul edilir.".into());
    }

    // URL'nin .exe ile bitmesi zorunlu
    if !url.to_lowercase().ends_with(".exe") {
        return Err("Güvenlik: Yalnızca .exe dosyaları indirilebilir.".into());
    }
    ...
    // SEC-2 FIX: Dosya boyutu makul sınırda mı? (50 MB üstünü reddet)
    const MAX_SIZE: usize = 50 * 1024 * 1024;
    if data.len() > MAX_SIZE {
        return Err("İndirilen dosya boyutu güvenlik sınırını aşıyor (50 MB).".into());
    }

    // SEC-2 FIX: SHA256 doğrulaması
    // TODO: check_update'ten dönen asset checksum'ı buraya taşı.
    // Şimdilik boş string → uyarı logu + geç (gelecek versiyon için altyapı hazır).
    if !verify_sha256(&data, "") {
        return Err("Güvenlik: İndirilen dosyanın SHA256 özeti eşleşmiyor. İndirme iptal edildi.".into());
    }

    // AppData\Local\beni-iceri-al-DPI'a kaydet
    let app_data = std::env::var("LOCALAPPDATA")
        .map_err(|_| "LOCALAPPDATA bulunamadı".to_string())?;
    let dir_name = obfuscate::reveal(crate::obf!("beni-iceri-al-DPI"));
    let exe_name = obfuscate::reveal(crate::obf!("beni-iceri-al-DPI_Update.exe"));
    let save_dir = format!("{}\\{}", app_data, dir_name);
    std::fs::create_dir_all(&save_dir)
        .map_err(|e| format!("Klasör oluşturulamadı: {}", e))?;

    let save_path = format!("{}\\{}", save_dir, exe_name);
...
/// İndirilen kurucuyu çalıştırır ve uygulamayı kapatır
/// SEC-1 FIX: Path doğrulaması — yalnızca uygulama dizinindeki .exe çalıştırılabilir.
#[tauri::command]
fn install_update(path: String) -> Result<(), String> {
    // SEC-1 FIX: Path traversal / arbitrary execution koruması
    let app_data = std::env::var("LOCALAPPDATA")
        .map_err(|_| "LOCALAPPDATA bulunamadı".to_string())?;
    let dir_name = obfuscate::reveal(crate::obf!("beni-iceri-al-DPI"));
    let expected_dir = format!("{}\\{}\\", app_data, dir_name);

    // Yol beklenen dizinle başlamalı ve .exe ile bitmeli
    if !path.starts_with(&expected_dir) {
        eprintln!("[SEC] install_update: Reddedilen yol: {}", path);
        return Err("Güvenlik: Kurulum dosyası geçersiz konumda.".into());
    }
    if !path.to_lowercase().ends_with(".exe") {
        return Err("Güvenlik: Yalnızca .exe dosyaları çalıştırılabilir.".into());
    }

    // Dosyanın var olduğunu doğrula
    if !std::path::Path::new(&path).exists() {
        return Err("Kurulum dosyası bulunamadı.".into());
    }

    let _ = std::process::Command::new(&path)
        .spawn()
        .map_err(|e| format!("Kurucu başlatılamadı: {}", e))?;
    // Uygulamayı kapat — kurucu güncelleme yapınca eski binary silinecek
    std::process::exit(0);
}
...
pub fn run() {
    // 🔐 Anti-debug: İlk katman — uygulama başlamadan hemen önce
    antidebug::run_detection();

    // P0-FIX: Single-instance enforcement — aynı anda sadece bir beni-iceri-al-DPI çalışabilir
    #[cfg(target_os = "windows")]
    {
        use std::ptr::null_mut;
        ...
        let mutex_name: Vec<u16> = (obfuscate::reveal(crate::obf!("Global\\beni-iceri-al-DPI_SingleInstance")) + "\0").encode_utf16().collect();

        unsafe {
            let handle = CreateMutexW(null_mut(), 0, mutex_name.as_ptr());
            if handle.is_null() || GetLastError() == ERROR_ALREADY_EXISTS {
                eprintln!("[STARTUP] ❌ beni-iceri-al-DPI zaten çalışıyor — çıkılıyor");

                use winapi::um::winuser::{
                    FindWindowW, IsIconic, SetForegroundWindow, ShowWindow, SW_RESTORE,
                };
                let window_name: Vec<u16> = "beni-iceri-al-DPI\0".encode_utf16().collect();
        ...
        .setup(|app| {
            #[cfg(desktop)]
            {
                ...
                let _tray = TrayIconBuilder::with_id("tray")
                    ...
                    .tooltip("beni-iceri-al-DPI - Kapalı")
